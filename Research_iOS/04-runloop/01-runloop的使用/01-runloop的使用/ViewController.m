...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    //Core
    NSLog(@"Core:%p",CFRunLoopGetMain());// 为什么这里打印的主线程对应的runloop和上边[NSRunLoop mainRunLoop]的不一样? 因为NSRunLoop是对CFRunLoop的包装,所以地址不一样,本质上地址还是用的CFRunLoop
...
- (void)run {
    //如何创建子线程对应的runloop? 使用currentRunloop懒加载的
