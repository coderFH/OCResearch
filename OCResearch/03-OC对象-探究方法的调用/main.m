...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        //这个调用顺序可以看到,NSObject分类中有个实例方法,
        //FHStudent在调用test的时候,通过类对象的isa会找到元类对象,看是否存储有test方法,没有就会通过superClass找到父类看看是否有test方法,如果还没有
        //会到根元素的元类中去找,如果还没有  此时会通过元类的superClass找到类对象,看是否有-号开头的test方法,有就会调用
        [FHStudent test];
    }
    return 0;
}
